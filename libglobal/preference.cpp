...
void Preference::setValue(const QString &key, const QVariant &data) { sInstance->mSetting->setValue(key, data); }

int Preference::getInt(const QString &key, int def) { return sInstance->mSetting->value(key, def).toInt(); }

QString Preference::getString(const QString &key, const QString &def) {
    return sInstance->mSetting->value(key, def).toString();
}

bool Preference::getBool(const QString &key, bool def) { return sInstance->mSetting->value(key, def).toBool(); }

double Preference::getDouble(const QString &key, double def) { return sInstance->mSetting->value(key, def).toDouble(); }
...
QString Preference::formatMoney(int val) { return QLocale().toString(val); }
